...
Symlinks are resolved before checking — a symlink inside an allowed
directory that points outside is still denied.
"""
...
def is_in_path_list(target: Path, path_list: list[str]) -> bool:
    ...
    Args:
        target: The path to check (will be resolved)
